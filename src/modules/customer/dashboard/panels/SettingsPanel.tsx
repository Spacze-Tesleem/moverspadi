...
export default function SettingsPanel({ isDark: D }: Props) {
  ...
  const Toggle = ({ on, onToggle }: { on: boolean; onToggle: () => void }) => (
    <button
      onClick={onToggle}
      className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${on ? "bg-blue-600" : D ? "bg-zinc-700" : "bg-slate-200"}`}
    >
  ...
  const Row = ({ icon: Icon, label, sub, children }: {
    icon: React.ComponentType<{ className?: string }>;
    label: string;
    sub?: string;
    children: React.ReactNode;
  }) => (
    <div className={`flex items-center gap-4 px-5 py-4 border-b last:border-0 ${D ? "border-white/5" : "border-slate-100"}`}>
      <div className={`p-2 rounded-xl shrink-0 ${D ? "bg-white/5" : "bg-slate-100"}`}>
        <Icon className={`w-4 h-4 ${D ? "text-zinc-400" : "text-slate-500"}`} />
      </div>
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-semibold ${D ? "text-zinc-200" : "text-slate-800"}`}>{label}</p>
        {sub && <p className={`text-xs mt-0.5 ${D ? "text-zinc-500" : "text-slate-400"}`}>{sub}</p>}
      </div>
      {children}
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`rounded-2xl border overflow-hidden ${D ? "border-white/5" : "border-slate-200"}`}>
        ...
        <Row icon={D ? Sun : Moon} label={D ? "Light Mode" : "Dark Mode"} sub="Switch the dashboard theme">
          <button
            onClick={toggleTheme}
      ...
      <div className={`rounded-2xl border overflow-hidden ${D ? "border-white/5" : "border-slate-200"}`}>
        ...
        <Row icon={Bell} label="Push Notifications" sub="Order updates and alerts">
          <Toggle on={prefs.pushNotifications} onToggle={() => toggle("pushNotifications")} />
        </Row>
        <Row icon={Globe} label="Email Updates" sub="Receipts and promotions">
          <Toggle on={prefs.emailUpdates} onToggle={() => toggle("emailUpdates")} />
        </Row>
        <Row icon={Smartphone} label="SMS Alerts" sub="Critical order updates only">
          <Toggle on={prefs.smsAlerts} onToggle={() => toggle("smsAlerts")} />
        </Row>
      </div>
      ...
      <div className={`rounded-2xl border overflow-hidden ${D ? "border-white/5" : "border-slate-200"}`}>
        ...
        <Row icon={Shield} label="Biometric Login" sub="Use fingerprint or Face ID">
          <Toggle on={prefs.biometricLogin} onToggle={() => toggle("biometricLogin")} />
        </Row>
        <Row icon={Shield} label="Location Services" sub="Required for booking">
          <Toggle on={prefs.locationServices} onToggle={() => toggle("locationServices")} />
        </Row>
