...
const DispatchForm = ({ bookingData, setBookingFormData, onNext }: Props) => {
  ...
  const updateItem = (idx: number, value: string) => {
    ...
    setBookingFormData(prev => ({ ...prev, items: updated }));
  };

  const addItem = () => {
    setBookingFormData(prev => ({ ...prev, items: [...items, { name: "", qty: 1, weight: "" }] }));
  };
      ...
      <div className="space-y-3">
        ...
        <div className={`grid ${isHaulage ? "grid-cols-1" : "grid-cols-2"} gap-3`}>
          {!isHaulage ? (
            ...
            <VehicleCard label="Heavy Truck" icon={Truck} active={bookingData.vehicleType === 'truck'} onClick={() => setBookingFormData(p => ({ ...p, vehicleType: 'truck' }))} />
          )}
        </div>
      </div>
...
// --- FORM: TOW ---
const TowForm = ({ bookingData, setBookingFormData, onNext }: Props) => {
...
// --- MAIN COMPONENT ---
export default function DetailsStep({ bookingData, setBookingFormData, onNext, onPrev }: Props) {
